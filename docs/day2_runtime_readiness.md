...
**Status:** R1-R4 frozen failed; final cross-split R5 implemented; R5 live canary not authorized

Amendment 008 records the terminal R4 NVIDIA invalid-label failure and excludes
that exact route as `excluded_protocol_noncompliant`. R1-R4 remain frozen with
`run_status=failed`; their successful outputs may not be reused. R5 has distinct
...
Qualifying active evidence exists for Groq `openai/gpt-oss-120b`, Gemini
`gemini-3.8-flash`, Cloudflare Workers AI
`@cf/qwen/qwen3-30b-a3b-fp8`, and OpenAI `gpt-5.6-terra`. NVIDIA evidence is
retained only as history and is not an active R5 input. Earlier failure reports
and all Mistral reports are rejected as runtime evidence. Mistral remains
`excluded_unavailable`.

## Cross-split R5 dry-run plans
...
The fixed totals are 51/204 for the three-item R5 canary, 1,020/4,080 for
...
Per item the provider allocation is Groq 11, Cloudflare Workers AI 3, Gemini 1,
...
Amendment 008. Split policy independently permits only smoke, and that does not
itself authorize an API call. The deterministic R5 three-item canary must be
...
The R5 canary uses run ID `cladder-smoke-canary-3-r5`, 51 logical calls, and a
204-transport-attempt ceiling. Its expected artifact root is
`artifacts/runs/cladder-smoke-canary-3-r5/`. Smoke-60 requires its frozen
...
R5 uses item-major execution and fixed run IDs. A planned
...
effective 2026-09-11. All active R5 provider/model pairs have official
normalized prices. NVIDIA's historical free-prototype entry remains
symbolic-null under Amendment 003 but is not an R5 execution or pricing input.
...
|---|---:|---:|
| R5 canary | 320 s | USD 0.01196158 |
...
R5 run has 100% normalized-price coverage when every active call reports usable
token counts. Groq, Gemini, Cloudflare Workers AI, and OpenAI may incur account
