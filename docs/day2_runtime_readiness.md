...
**Status:** R1-R5 frozen failed; post-stop cross-split R6 implemented; R6 live canary not authorized

Amendment 009 records the four R5 Gemini HTTP 503/UNAVAILABLE failures and excludes
that route as `excluded_transient_unavailable_r5`. NVIDIA retains its prior exclusion.
R1-R5 remain frozen with
`run_status=failed`; their successful outputs may not be reused. R6 has distinct
run identities and may not resume or merge any earlier canary artifact.
...
Qualifying active evidence exists for Groq `openai/gpt-oss-120b`, Cloudflare Workers AI
`@cf/qwen/qwen3-30b-a3b-fp8`, and OpenAI `gpt-5.6-terra`. Gemini and NVIDIA evidence is
retained only as history and is not an active R6 input. Earlier failure reports
and all Mistral reports are rejected as runtime evidence. Mistral remains
`excluded_unavailable`.

## Cross-split R6 dry-run plans
...
The fixed totals are 51/204 for the three-item R6 canary, 1,020/4,080 for
...
Per item the provider allocation is Groq 11, Cloudflare Workers AI 4, Gemini 0,
OpenAI 2, and NVIDIA NIM 0.

| Provider | Logical API calls |
|---|---:|
| Groq | 660 |
| Gemini | 0 |
| Cloudflare Workers AI | 240 |
| OpenAI | 120 |
| NVIDIA NIM | 0 |
...
Amendment 009. Split policy independently permits only smoke, and that does not
itself authorize an API call. The deterministic R6 three-item canary must be
separately run with `--authorize-live-smoke` and pass before smoke-60 can be
considered. A failed canary never starts smoke-60 automatically. R6 is the final revision
for today's session; do not automatically create R7.
Calibration-300 and locked-test-600 remain `BLOCKED_NOT_AUTHORIZED` under their
separate acknowledgement flags.

The R6 canary uses run ID `cladder-smoke-canary-3-r6`, 51 logical calls, and a
204-transport-attempt ceiling. Its expected artifact root is
`artifacts/runs/cladder-smoke-canary-3-r6/`. Smoke-60 requires its frozen
complete result, 51/51 calls with zero terminal errors, and a new authorization.
The R6 canary itself requires the exact R5 manifest/tree/summary hashes in
Amendment 009; the execution core rechecks that lineage before any runtime write. Calibration then requires a new
...
R6 uses item-major execution and fixed run IDs. A planned
...
effective 2026-09-11. All active R6 provider/model pairs have official
normalized prices. NVIDIA's historical free-prototype entry remains
symbolic-null under Amendment 003. Gemini and NVIDIA are historical data only,
not R6 execution or pricing inputs.
Actual account charges remain separate nullable fields; free-tier billing never
forces normalized list cost to zero.
...
|---|---:|---:|
| R6 canary | 320 s | USD 0.01196158 |
| Smoke-60 | 6,590 s | USD 0.23923167 |
| Calibration-300 | 32,990 s | USD 1.19615833 |
| Locked-test-600 | 65,990 s | USD 2.39231667 |

These are pacing lower bounds and Groq-only projections, not full-run cost
forecasts. Dry-run has no token usage, so its total cost remains `null`; a live
R6 run has 100% normalized-price coverage when every active call reports usable
token counts. Groq, Cloudflare Workers AI, and OpenAI may incur account
charges. Direct actual-charge evidence is unavailable, so
`actual_charge_usd` remains separate and nullable.
