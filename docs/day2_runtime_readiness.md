...
**Status:** R1 frozen failed; cross-split R2 implemented; R2 live canary not authorized

Amendment 005 supersedes the Amendment 004 execution revision. The original
`cladder-smoke-canary-3` remains frozen with `run_status=failed`; none of its 27
successful outputs may be reused. R2 has distinct run identities and may not
resume or merge R1 artifacts.
...
The fixed totals are 51/204 for the three-item R2 canary, 1,020/4,080 for
...
Amendment 005. Split policy independently permits only smoke, and that does not
itself authorize an API call. The deterministic R2 three-item canary must be
...
The R2 canary uses run ID `cladder-smoke-canary-3-r2`, 51 logical calls, and a
204-transport-attempt ceiling. Its expected artifact root is
`artifacts/runs/cladder-smoke-canary-3-r2/`. Smoke-60 requires its frozen
...
R2 uses item-major execution and fixed run IDs. A planned
...
|---|---:|---:|
| R2 canary | 320 s | USD 0.01196158 |
