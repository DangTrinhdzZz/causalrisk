...
def test_smoke_dry_run_is_deterministic_and_counts_alias_once(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(dry_run, "_r3_artifact_state", lambda _path: "create")
    ...
    assert plan.run_id == "cladder-smoke-60-r3"
    assert Path(plan.artifact_path).name == "cladder-smoke-60-r3"
...
def test_three_item_canary_has_one_item_per_rung_and_51_calls(monkeypatch):
    ...
    assert plan.run_id == "cladder-smoke-canary-3-r3"
    assert plan.provider_calls == {
...
def test_dry_run_rejects_ambiguous_in_progress_artifact(tmp_path):
    run_dir = tmp_path / "cladder-smoke-60-r3"
    run_dir.mkdir()
    ...
    with pytest.raises(ValueError, match="ambiguous in-progress"):
        dry_run._r3_artifact_state(run_dir)


@pytest.mark.parametrize("run_id", ["cladder-smoke-canary-3", "cladder-smoke-canary-3-r2"])
def test_terminal_legacy_canaries_are_never_resumed(tmp_path, run_id):
    ...
    assert dry_run._r3_artifact_state(run_dir) == "terminal_frozen_no_resume_no_overwrite"
